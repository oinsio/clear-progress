# Оптимизация синхронизации — проверка изменений перед установкой _dirty

## Что изменилось

Добавлена проверка реальных изменений данных перед пометкой записи как `_dirty`. Теперь запись помечается для синхронизации только если её поля действительно изменились.
...
**Логика:**
- Если данные не изменились: `_dirty = false`, `version` и `updated_at` остаются прежними
- Если данные изменились: `_dirty = true`, `version` инкрементируется, `updated_at` обновляется
...
3. Помечают как `_dirty` только те записи, у которых `sort_order` действительно изменился
...
Методы `TaskService.complete()` и `TaskService.noncomplete()` всегда помечают запись как `_dirty`, т.к. это явное изменение состояния задачи.
...
// ❌ Запись помечается _dirty, отправляется на сервер
```
...
// Пользователь открыл задачу, ничего не изменил, закрыл
await taskService.update(taskId, {});
// ✅ Запись НЕ помечается _dirty, запрос на сервер не отправляется
```
