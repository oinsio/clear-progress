...
Устройство A удаляет, устройство B редактирует. Last-write-wins: если B позже — запись "воскреснет". Для персонального GTD-приложения это приемлемо. Не нужно усложнять логику правилом "delete всегда побеждает".
