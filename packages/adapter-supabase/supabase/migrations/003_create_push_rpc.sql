  ...
  FOR v_rec IN SELECT value FROM jsonb_array_elements(p_tasks) AS value LOOP
    ...
    IF NOT FOUND THEN
      ...
      VALUES (
        ...
        COALESCE(v_rec->>'repeat_rule', ''), (v_rec->>'is_hidden')::BOOLEAN,
    ...
    ELSIF (v_rec->>'updated_at')::TIMESTAMPTZ >= v_existing_ts THEN
      UPDATE tasks SET
        ...
        repeat_rule = COALESCE(v_rec->>'repeat_rule', ''), is_hidden = (v_rec->>'is_hidden')::BOOLEAN,
    ...
    ELSE
      SELECT jsonb_build_object(
        ...
        'repeat_rule', repeat_rule, 'is_hidden', is_hidden,
