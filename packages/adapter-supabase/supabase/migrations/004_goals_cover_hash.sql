-- implements FR1, FR7 of content-addressable-covers
-- Replace cover_file_id UUID FK with cover_hash TEXT on goals table,
-- and update push_records RPC to use cover_hash instead of cover_file_id.

-- ── Step 1: add cover_hash column ────────────────────────────────────────────

ALTER TABLE goals ADD COLUMN cover_hash TEXT NOT NULL DEFAULT '';

-- ── Step 2: populate cover_hash from existing covers join ─────────────────────

UPDATE goals
SET cover_hash = c.data_hash
FROM covers c
WHERE goals.cover_file_id = c.file_id;

-- ── Step 3: drop FK constraint and cover_file_id column ───────────────────────

ALTER TABLE goals DROP CONSTRAINT goals_cover_file_id_fkey;
ALTER TABLE goals DROP COLUMN cover_file_id;

-- ── Step 4: update push_records to use cover_hash ────────────────────────────
