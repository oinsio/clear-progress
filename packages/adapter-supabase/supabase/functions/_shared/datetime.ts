...
/**
 * Ensures a timestamp string has millisecond precision (.000 appended if missing).
 */
function ensureMilliseconds(isoString: string): string {
  // Already has fractional seconds: "...T10:30:00.123Z"
  if (/\.\d+Z$/.test(isoString)) return isoString;
