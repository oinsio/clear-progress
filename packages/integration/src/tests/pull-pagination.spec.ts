...
test("initial pull with record count > max_rows fetches all records", async () => {
  ...
  // Step 2: Generate and push 15 tasks directly via pushToServer
  const pushedTaskIds: string[] = [];
  const tasksToCreate = Array.from(
    { length: PAGINATION_TEST_TASK_COUNT },
    (_, index) => {
      const taskId = randomUUID();
      pushedTaskIds.push(taskId);
      return {
        id: taskId,
        name: `Pagination Task ${index + 1}`,
        description: "",
        box: "inbox",
        is_completed: false,
        is_deleted: false,
...
test("incremental pull after partial batch — all records fetched", async () => {
  ...
  // Step 2: Push 12 more tasks with unique names
  const incrementalTaskIds: string[] = [];
  const incrementalTasks = Array.from(
    { length: INCREMENTAL_TASK_COUNT },
    (_, index) => {
      const taskId = randomUUID();
      incrementalTaskIds.push(taskId);
      return {
        id: taskId,
        name: `Incremental Task ${index + 1}`,
        description: "",
        box: "inbox",
        is_completed: false,
        is_deleted: false,
...
test("crash-recovery — re-pull from last saved revision fetches all records", async () => {
  ...
  // Step 2: Push 15 more tasks to ensure pagination is needed (max_rows=10)
  const crashRecoveryTaskIds: string[] = [];
  const crashRecoveryTasks = Array.from(
    { length: CRASH_RECOVERY_TASK_COUNT },
    (_, index) => {
      const taskId = randomUUID();
      crashRecoveryTaskIds.push(taskId);
      return {
        id: taskId,
        name: `CrashRecovery Task ${index + 1}`,
        description: "",
        box: "inbox",
        is_completed: false,
        is_deleted: false,
