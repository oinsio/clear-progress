Feature: Sync Orchestration — Triggers
  ...
  Scenario: Sync runs when tab becomes visible
    Given SyncProvider has mounted and initial sync completed
    When the tab becomes visible
    Then a sync cycle is executed

  @sync-orchestration @T7
  Scenario: Sync runs when window receives focus
    Given SyncProvider has mounted and initial sync completed
    When the window receives focus
    Then a sync cycle is executed

  @sync-orchestration @T8
  Scenario: Sync runs on bfcache restore
    Given SyncProvider has mounted and initial sync completed
    When pageshow fires with persisted=true
    Then a sync cycle is executed
