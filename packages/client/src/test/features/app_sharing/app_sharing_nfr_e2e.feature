Feature: App Sharing — Accessibility and Responsiveness (E2E)
  ...
  Scenario: Copy link button has proper aria-label
    Then copy link button has a non-empty aria-label
  ...
  @share-with-friend @NFR-A2
  Scenario: Confirmation dialog OK button is activatable via Enter
    When user clicks the copy link button
    Then confirmation dialog is visible
