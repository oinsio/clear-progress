Feature: Attachment repository sync
  ...
  Scenario: Get attachments needing sync
    Given two attachments with needsSync true exist
    And one attachment with needsSync false exists
  ...
  Scenario: Apply server records
    ...
    Then both records are stored with needsSync false

  @add-file-attachments @FR5
  Scenario: Apply server records skips local dirty records
    Given a local attachment with needsSync true exists
