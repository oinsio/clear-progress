...
describeFeature(feature, (f: FeatureDescriibeCallbackParams<Context>) => {
  ...
  f.Scenario(
    ...
    ({ Given, When, Then, And }) => {
      ...
      Then(
        'task "T1" has is_deleted true and syncStatus "pending"',
        async (_ctx: TestContext) => {
      ...
      And(
        'checklist item "C1" has is_deleted true and syncStatus "pending"',
        async (_ctx: TestContext) => {
      ...
      And(
        'checklist item "C2" has is_deleted true and syncStatus "pending"',
        async (_ctx: TestContext) => {
          const item = await db.checklist_items.get(
  ...
  f.Scenario(
    ...
    ({ Given, When, Then, And }) => {
      ...
      And(
        'checklist item "C2" has is_deleted true and syncStatus "pending" with updated updated_at',
        async (_ctx: TestContext) => {
          const item = await db.checklist_items.get(
            getIdOrThrow(ctx.checklistItemIds, "C2"),
          );
