...
import { getAllIdeas, getIdeasByRevision, deleteIdeasByIds, upsertIdeas } from './ideas.sheet';
...
function makeIdeaRow(overrides: Partial<Record<string, unknown>> = {}): unknown[] {
...
describe('getAllIdeas', () => {
  ...
  it('should skip rows where first column is empty', () => {
    ...
    vi.mocked(getSheet).mockReturnValue(makeSheetMock([IDEA_HEADERS, emptyRow]) as never);

    expect(getAllIdeas()).toEqual([]);
  });

  it('should return one idea when sheet has one data row', () => {
    vi.mocked(getSheet).mockReturnValue(makeSheetMock([IDEA_HEADERS, makeIdeaRow()]) as never);
  ...
  it('should coerce null row values to empty string for string fields', () => {
    vi.mocked(getSheet).mockReturnValue(makeSheetMock([
      IDEA_HEADERS,
      makeIdeaRow({ name: null, description: null, created_at: null, updated_at: null }),
    ]) as never);
...
describe('deleteIdeasByIds', () => {
  ...
  it('should return 0 when ids array is empty', () => {
    const sheetMock = makeSheetMock([IDEA_HEADERS, makeIdeaRow({ id: 'idea-1' })]);
