...
describe('recordToRow - date handling', () => {
  ...
  it('should convert Date object in next_date to prefixed ISO date', () => {
    const task = makeTask({ next_date: new Date('2026-04-20T00:00:00Z') as any });
    const row = recordToRow(SHEET_NAMES.TASKS, task);
...
describe('upsertRecords - row isolation', () => {
  ...
  it('should call setValues separately for each updated row', () => {
    ...
    const existingData = [
      ['id', 'name', 'description', 'box', 'goal_id', 'context_id', 'category_id', 'is_completed', 'completed_at', 'repeat_rule', 'is_hidden', 'next_date', 'appear_date', 'original_task_id', 'sort_order', 'is_deleted', 'created_at', 'updated_at', 'version', 'revision'],
      ['task-1', 'Task 1', '', 'inbox', '', '', '', false, '', '', false, '', '', '', 0, false, '2025-01-01T00:00:00.000Z', '2025-01-01T00:00:00.000Z', 1, 0],
      ['task-2', 'Task 2', '', 'inbox', '', '', '', false, '', '', false, '', '', '', 0, false, '2025-01-01T00:00:00.000Z', '2025-01-01T00:00:00.000Z', 1, 0],
    ];
