...
- **MODIFIED**: Ref-counting now counts references from both `goals.cover_hash` and `attachments.data_hash`
...
- FR7: Ref-counting SHALL consider references from both `goals.cover_hash` and `attachments.data_hash` when deciding whether to delete a file from server storage.
...
- `cover-sync-protocol`: Renamed to generic file operations (uploadFile, getFile, deleteFile). Ref-counting expanded to include attachment references.
...
- M3: Attachments sync reliably between devices (create, delete)
