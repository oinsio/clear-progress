...
A recurring copy MUST be created with is_hidden=true if appear_date is after today. Copy MUST be created with is_hidden=false if appear_date is today or in the past. Hidden tasks MUST NOT appear in box lists, search, or association queries.

#### Scenario: Recurring copy hidden when appear_date is future
- **GIVEN** today is "2026-01-15" and calculated appear_date is "2026-01-20"
...
- **GIVEN** today is "2026-01-15" and calculated appear_date is "2026-01-15"
- **WHEN** system creates a recurring copy
- **THEN** copy has is_hidden false

#### Scenario: Recurring copy visible when appear_date is past
- **GIVEN** today is "2026-01-20" and calculated appear_date is "2026-01-15"
- **WHEN** system creates a recurring copy
- **THEN** copy has is_hidden false
...
System MUST reveal hidden tasks (set is_hidden=false, mark for sync) when appear_date <= today. Reveal MUST be triggered on: app mount, midnight transition, sync_complete event, and return from background (visibility change).

#### Scenario: Reveal tasks whose appear_date has arrived
- **GIVEN** hidden task with appear_date "2026-01-15" and today is "2026-01-15"
...
- **GIVEN** hidden task with appear_date "2026-01-20" and today is "2026-01-15"
- **WHEN** system runs reveal check
- **THEN** task remains hidden

#### Scenario: Reveal triggered on app mount
- **WHEN** app mounts
- **THEN** system runs reveal check

#### Scenario: Reveal triggered at midnight
