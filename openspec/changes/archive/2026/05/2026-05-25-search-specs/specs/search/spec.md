...
useSearch SHALL execute search across tasks, goals, and ideas in parallel via Promise.all. All three entity services' searchByName methods SHALL be called with the same query string. Results SHALL be stored separately by entity type. Implements FR1 of search-specs.
...
Empty query (empty string) SHALL clear all results immediately without calling any search service. Implements FR2 of search-specs.
...
If any of the three search services throws an error, ALL results SHALL be cleared (tasks, goals, ideas set to empty arrays). The error SHALL be logged to console. Implements FR3 of search-specs.
...
clear() method SHALL reset tasks, goals, and ideas to empty arrays. Implements FR4 of search-specs.
...
isSearching SHALL be true while search is in progress and false after completion (both success and error). isSearching SHALL be false initially. Implements FR5 of search-specs.
