...
BottomNav SHALL render exactly five navigation items in order: Inbox, Today, Goals, Ideas, Search. Each item SHALL link to its corresponding route from the ROUTES constant.  # implements FR1, FR2 of app-shell-navigation-spec
...
The currently active navigation item SHALL have aria-current="page" set by NavLink. Non-active items SHALL NOT have aria-current.  # implements FR3 of app-shell-navigation-spec
...
BottomNav SHALL use i18n translation keys (nav.*) for all labels. The nav element SHALL have an aria-label from i18n.  # implements FR8, FR10 of app-shell-navigation-spec
...
Each navigation item SHALL render a Lucide icon with aria-hidden="true" to prevent screen reader duplication.  # implements FR9, NFR-A2 of app-shell-navigation-spec
...
The root path "/" SHALL redirect to the Inbox route (ROUTES.INBOX = "/tasks").  # implements FR4 of app-shell-navigation-spec
...
All page routes SHALL be wrapped in AppLayout (AppShell + Outlet). Today, Week, and Later routes SHALL additionally be wrapped in PageLayout (PageShell + Outlet with BottomNav).  # implements FR5, FR6 of app-shell-navigation-spec
...
Route errors SHALL be caught by RouteErrorFallback (errorElement on the AppLayout route). The error SHALL be logged to console and an ErrorFallback component SHALL be shown.  # implements FR7 of app-shell-navigation-spec
...
BottomNav SHALL render a `<nav>` element with a descriptive aria-label for accessibility.  # implements NFR-A1 of app-shell-navigation-spec
...
BottomNav SHALL be visible on mobile viewports (below md breakpoint) and hidden on desktop viewports (md and above) via the md:hidden CSS class on the wrapper in PageShell.  # implements NFR-R1 of app-shell-navigation-spec
