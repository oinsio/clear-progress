...
- **MODIFIED**: Recurring-copy deduplication after pull merges instead of blindly keeping the earliest copy: schedule fields (`next_date` + `appear_date`, always as a pair) come from the copy with the earliest `next_date`, content fields from the copy with the freshest `updated_at`; if `repeat_rule` differs between the schedule winner and the freshest-`updated_at` copy, the freshest copy wins wholesale.
...
- `repeating-tasks`: "System reveals hidden tasks when appear date arrives" no longer refreshes `updated_at`; "System deduplicates recurring copies after pull" changes from keep-earliest-copy to merge (earliest schedule pair + the freshest content, freshest-wholesale when `repeat_rule` differs between the schedule winner and the freshest-`updated_at` copy).
