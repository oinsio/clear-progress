...
- No runtime code changes; the i18n mechanism and i18n-check tooling stay as-is.
...
_None. The `i18n` and `i18n-check` capabilities already support dialect locales with fallback; only locale content changes._
...
- NG1: No changes to i18n runtime, locale registry, or fallback mechanics.
- NG2: No changes to `ru.json` / `en.json` base locales.
