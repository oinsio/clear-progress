...
- [ ] 1.1 Write BDD feature file `packages/client/src/test/features/repeating_tasks/dedup_after_pull.feature` with scenarios from delta spec (@dedup-recurring-after-pull @FR1 @FR2 @FR3)
- [ ] 1.2 Write step definitions `dedup_after_pull.steps.ts`
- [ ] 1.3 RED: Write unit tests for `RecurringTaskDeduplicator.deduplicate()`:
...
- [ ] 1.4 GREEN: Implement `RecurringTaskDeduplicator` in `packages/client/src/services/RecurringTaskDeduplicator.ts`
...
- [ ] 1.5 REFACTOR: Clean up, verify all tests green
...
- [ ] 2.1 RED: Write test for `TaskRepository.findDuplicateRecurringGroups(originalTaskIds: string[])` — returns groups of non-completed, non-deleted tasks sharing `original_task_id`
- [ ] 2.2 GREEN: Implement the method using Dexie `where("original_task_id").anyOf(ids)` with filter
- [ ] 2.3 REFACTOR + tests green
...
- [ ] 3.1 RED: Write test for `SyncService._pull()` — dedup called after batch, before `sync_complete`
- [ ] 3.2 RED: Write test — dedup skipped when no tasks with `original_task_id` in batch (FR5)
- [ ] 3.3 GREEN: Wire `RecurringTaskDeduplicator` into `SyncService`
...
- [ ] 4.1 Change assertion in `packages/integration/src/tests/multi-device-recurring.spec.ts` test 5.13.3: `toBeGreaterThanOrEqual(1)` → `toHaveLength(1)`
