# Capability: GAS Adapter

## Purpose

Client-side adapter implementing `SyncAdapter` interface for communication with Google Apps Script web app backend. Handles HTTP request formatting, authentication token injection, response validation via Zod schemas, timeout management, and error classification.
