...
Defines the user-configurable sync-timing preferences — the periodic sync interval and the post-edit auto sync (debounce) delay — including their storage, validation, self-healing, sync behavior, and the Account & Sync section controls that expose them.
